...
| AD-007 | macOS Rockchip 组件 entitlement 死锁：运行时校验器要求 app-sandbox+inherit 与打包契约(#1052)要求空 entitlements 互斥，以修改校验器收口 | 2026-08-04 定案；#1299 helper signing 现代化；[AFD-0003](../decisions/AFD-0003-arkforged-signing-packaging.md) | C + A(2026-08-16 本机实测) | **resolved** — NRU-004 后发布包只含 `arkforged`，其 entitlement 字典为空；`crates/arkforged/src/packaging.rs` 在 daemon 绑定自身 toolchain 身份前读 Mach-O 并要求字典为空，fat 的每个 slice 都判(vendor fallback 已随 NRU-004 删除，同一 reader 现在守的是 native 绑定)。App Sandbox 键在 development 与 release 两个模式下都拒——不存在「这个构建里 sandbox 子进程能跑」的情况 |
...
| AD-022 | **AD-015 的启动自检。** 绑定的工具必须先通过摘要比对，再**证明它能跑**：以 device-free 的 `-v` 探测，带 5 秒超时。2026-08-15 复现并诊断了完整的 AD-015 情形——同一份字节(`bbd7bdc0…`)带 quarantine 时探测 5,009 ms 未返回、被杀掉并拒绝启动，错误里直接给出 `xattr -d com.apple.quarantine <path>`;清除 quarantine 后**摘要一字不差**、自检 207 ms 通过、`execution: ready`。另实测到 quarantine 的**第二种形态**:并非总是挂死，也可能立即退出且无任何输出——两种形态都被拒，且都附带 quarantine 证据(`/usr/bin/xattr` 尽力查询，查不到就明说查不到) | `crates/arkforged/src/dispatch.rs` `HostFixedToolPort::self_test`(该 port 与自检已随 NRU-004 删除——不再有外部工具可绑;本行是 2026-08-15 的历史实测);四条真实启动路径 | A(本机实测) | confirmed(历史;机制已退役，教训仍成立) |
| AD-023 | **ArkDeck 当时钉给破坏性 flash 的那份工具不可出厂。** `pinnedProduction` = `038a8a0e…3611`(= 2026-08-15 彩排跑的那份)的动态库闭包里有 `/opt/homebrew/opt/libusb/lib/libusb-1.0.0.dylib`——一个只有这台机器上才有的路径。ArkDeck.app 内捆绑的密闭构建 `231a05ef…c79e` 把 libusb 静态链入(AD-014 的 recipe)，闭包只剩七个系统库，且 Developer ID + Hardened Runtime + 空 entitlement 字典俱全，是三份 `rkdeveloptool` 里**唯一**满足旧 AFD-0003 vendor release 条款的一份 | 本机 `otool -L` 三份逐一实测；`packaging/macos/package-arkforged.sh`；NRU-004 native 默认与打包静态契约 | A(本机实测) + C | **resolved(NRU-004)** — 不可出厂依赖事实不变；解法是发布包彻底不携带 vendor 二进制，默认 toolchain 身份收敛为 arkforged 自身摘要，而不是放宽动态库闭包或换另一份 vendor pin。历史 toolchain 摘要后果保留在下节 |
...
| AD-033 | **真机全量刷写两过：首过与原生换轨。** (a) 2026-08-18 `job-a4b7d539…` 端到端 `succeeded`——AF-V2 第一验收项通过，执行面为当日 fixed-tool 运行时(daemon `aa7fe808…0085` 含 vendor 字符串；回执 provider 锚 = 捆绑构建 `231a05ef…c79e`)；(b) NRU-001..004 同日起步：原生读写落地、真机读 parity 入仓、默认切换、`c049a11` 移除 vendor 运行时；(c) 2026-08-19 `job-b00e006a…` 纯原生二进制(`f3dfc624…66d9`，零 vendor 字符串，回执自证该摘要)全量复验 `succeeded`。两次 readback 证据逐分区同形(2 Verified / 1 结构性 Failed(`boot_linux` 跨读窗边界) / 6 TypedSkip，与 AD-006/AD-019 相容)，postflight 均 `exact-published-profile-and-bound-hdc`(设备答 `OpenHarmony-7.0.0.37`) | [首过与原生换轨](runs/2026-08-19-dayu200-green-flash-and-native-cutover.md)；arkforged journal `JOB-000001A013991062` / `JOB-000001A0180894B7` | A(真机) | confirmed；每次通过只发布自己那一个 maturity 组合，ProductionVerified 提升是另一个决定(AD-025) |
...
> 编号说明：AD-029..AD-032 是 2026-08-18 flash bring-up 期间以 commit 为载体的
> 缺陷修复记录(`515c86d` 时钟冻结、`e4ffad0` 控制回执证据规则、`de8d0b4`
> 控制期限清扫、`aed9873` 回执截断保尾)，未单独入表；其中 `aed9873` 的
> `toolOutputTail` 词汇已被 NRU-004 的原生回执词汇(`operationDetailTail`)取代。
...
> 2026-08-19 注：这条义务曾由 AD-022 的启动自检实现并实测。NRU-004 之后
> 不再有外部工具可绑，「验证工具可执行」的 preflight 随之消失；教训本身
> (哈希相等 ≠ 可运行)保留为宿主环境类失败的一般原则。
...
> 2026-08-19 注：第 1 条按预言发生了。2026-08-18 的首次真机写入(AD-033 之 a)
> 用的是 `231a05ef…c79e`——三份里唯一可出厂的那份，故它的通过发布的是可出厂
> 组合；`038a8a0e…` 至终无写入证据。随后 NRU-004 换的不是另一份 vendor pin
> 而是原生实现：toolchain 身份收敛为 `arkforged` 自身摘要，「另一个组合需要
> 另一次真机通过」同样按预言执行——2026-08-19 纯原生二进制全量复验
> (AD-033 之 c)就是那一次。彩排的设备侧事实(读域/分区表/rebind)继续有效。
