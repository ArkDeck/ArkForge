...
| AD-011 | `pinnedReadOnlyDiscovery`(`bbd7bdc0…`，本机 = `/opt/homebrew/bin/rkdeveloptool`)在本机**不可用**：`ld` 与 `-v` 均挂起，设备处于 HDC-normal 或 Loader 皆然，即无设备操作也挂起 | [同上](runs/2026-08-14-dayu200-read-only-capture.md) §6 | A(本机实测) | **open** — ArkDeck 只读发现路径钉的正是这个二进制 |
| AD-012 | ArkDeck.app 捆绑的 rkdeveloptool(`231a05ef…`，剥签名后 `c31e8a3f…`)与**两个 pin 均不符**，且剥签名后也不等于 rehearsal 构建(`2081fb90…`)——它是第三个构建 | [同上](runs/2026-08-14-dayu200-read-only-capture.md) §6 | A(本机实测) | **open** — 捆绑组件(CHG-2026-036)与 pin 的关系未声明 |
| AD-013 | `rkdeveloptool ld` 对处于 **HDC-normal** 的 DAYU200 报告 `Mode=Maskrom`(PID `0x5000`，"HDC Device"，HDC 正常应答)——连续三次复现。Loader 模式下报 `Mode=Loader` 正确 | [同上](runs/2026-08-14-dayu200-read-only-capture.md) §6.1 | A(本机实测) | **open** — 依赖 `ld` 的 Mode 字段判定模式的路径会把运行中的系统误判为 Maskrom |
